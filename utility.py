...
def GetProcId(processName):
    ...
    if (hSnap != INVALID_HANDLE_VALUE):
        procEntry = PROCCESSENTRY32()
        procEntry.dwSize = ctypes.sizeof(PROCCESSENTRY32)
...
def FindMyDMAAddy(hProc, base, offsets, arch=64):
    size = 8
    if (arch==32): size = 4
    ...
    for offset in offsets:
        ...
        address = ctypes.c_uint(address.value + offset)
...
def patchBytes(handle, src, destination, size):
    ...
    kernel32.VirtualProjectEx(handle, destination, size, PAGE_EXECUTE_READWRITE, ctypes.byref(oldProtect))
